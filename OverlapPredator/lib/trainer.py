...
class Trainer(object):
    ...
    def inference_one_batch(self, inputs, phase):
        ...
        if (phase == 'train'):
            ...
            c_loss = stats['circle_loss'] * self.w_circle_loss + stats['overlap_loss'] * self.w_overlap_loss + \
            ...
            with torch.no_grad():
                ...
                src_points = inputs['src_pcd_raw'].cpu().numpy()  # shape (N, 3)
                tgt_points = inputs['tgt_pcd_raw'].cpu().numpy()  # shape (M, 3)

                rot = inputs['rot'].cpu().numpy()  # shape (3, 3)
                trans = inputs['trans'].cpu().numpy().reshape(3)  # shape (3,)
                ...
                # Print transformed source and target points
                ...
                o3d.visualization.draw_geometries([
                    src_pcd.paint_uniform_color([1, 0, 0]),  # red = transformed source
    ...
    def inference_one_epoch(self, epoch, phase):
        ...
        c_loader_iter = iter(self.loader[phase])
        ...
        for c_iter in tqdm(range(num_iter)):
            ...
            if (c_iter + 1) % self.verbose_freq == 0 and self.verbose:
                ...
                for key, m in stats_meter.items():
                    self.writer.add_scalar(f'{phase}/{key}', m.avg, curr_iter)
                message = f'{phase} Epoch: {epoch} [{c_iter+1:4d}/{num_iter}]'
                for key, m in stats_meter.items():
                    message += f'{key}: {m.avg:.2f}\t'
