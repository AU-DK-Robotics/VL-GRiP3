...
def load_grasp_points():
    STROKE = 0.085   # 85 mm max opening
    PALM_Z = 0.01   # thickness of the palm
    FINGER_Z = 0.038   # fingertip is 75 mm beyond the palm

    OFFSET_Z = -0.06

    half_width = STROKE / 2.0

    control_points = np.array([
        [ half_width,  0.0,       PALM_Z+OFFSET_Z,   1.0 ],
        [-half_width,  0.0,       PALM_Z+OFFSET_Z,   1.0 ],
        [ half_width,  0.0,     FINGER_Z+OFFSET_Z,   1.0 ],
        [-half_width,  0.0,     FINGER_Z+OFFSET_Z,   1.0 ],
    ], dtype=np.float32)

    mid = (control_points[0] + control_points[1]) / 2.0

    grasp_pc = [
        control_points[-2],
        control_points[0],
        mid,
        [0.0, 0.0, OFFSET_Z, 1.0],
        mid,
        control_points[1],
        control_points[-1],
    ]
