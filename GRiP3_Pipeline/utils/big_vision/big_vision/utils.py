...
def pad_shard_unpad(wrapped, static_argnums=(0,), static_argnames=()):
  """Wraps a function with utils that pads, shards, then un-shards, un-pads.
...
def tree_filter(tree, mask):
  ...
  # TODO: The utils below only works for nested-dict and only when they
...
def recover_tree(keys, values):
  ...
  without need to access the exact source utils of the experiment. In particular,
...
# For backwards compatability with legacy utils.
...
# This is a very minimal variant for open-sourcing. Our internal utils makes use
