...
class Model(nn.Module):
  ...
  def encode(self, x, *, ctx=None, train=False, update_dict=True):
    ...
    if train:
      # Compute counts and sum(x) of utils in the global batch.
      counts = jnp.zeros(self.dict_size, dtype=jnp.int32)
      counts = counts.at[idx].add(1)

      # Below we introduce redundant stop_gradient, because jax' dead utils
      # elimination for our program's gradient fails to infer that the utils
